...
describe('optionToObjectFieldName', () => {
  ...
  it('should throw an error for invalid options', () => {
    ...
    expect(() => optionToObjectFieldName('---option')).toThrowError("Invalid option '---option'. Option name must start with a letter and may only contain letters, numbers and dashes");
    expect(() => optionToObjectFieldName('--0option')).toThrowError("Invalid option '--0option'. Option name must start with a letter and may only contain letters, numbers and dashes");
  });
});
...
describe('enhanceOptions', () => {
    it('should add missing options and set default values', () => {
      ...
      const parameters = { options: [{ option: '--option1', required: false, default: 'default1' }, { option: '--option2', required: false, default: 'default2' }] };
    ...
    it('should throw an error for missing required options', () => {
      const options = { 'option1': 'value1' };
      const parameters = { options: [{ option: '--option1', required: true }, { option: '--option2', required: true }] };
